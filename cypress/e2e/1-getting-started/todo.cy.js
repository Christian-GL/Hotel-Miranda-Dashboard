...
describe('test login', () => {
  ...
  it('Redirige a [localhost:5173/] si el usuario NO está autenticado y hace clic en un enlace a una ruta privada estando dentro de la aplicación', () => {
    ...
    cy.url().should('eq', 'http://localhost:5173/bookings')

    // cy.get('[data-cy=nav-ctn-bookings]').click()
    cy.wait(500)
    cy.get('[data-cy=nav-ctn-bookings]', { timeout: 10000 }).should('be.visible').click()
    cy.url().should('eq', 'http://localhost:5173/')
  })
